...
```{r ec-lt-fixed}
assay(se, "logCPM") <- cpm(assay(se, "counts"), log = TRUE, lib.sizes = se$totals)
...
p <- lt_results %>% 
  ...
  geom_smooth(se = FALSE) +
  geom_hline(yintercept = 0) +
  ggtitle(glue("{target}: extraChIPs"), subtitle = "Fixed-Width; Limma-Trend") +
  labs(colour = "Status") +
  scale_colour_manual(values = status_colours)
if (interactive()) {
  p
...
p <- all_res %>%
  ...
  arrange(desc(p)) %>% 
  mutate(
    windows = ifelse(grepl("^SW", method), "Sliding\nWindows", "Fixed\nWidth"),
    groups = ifelse(grepl("G$", method), "Explicit\nGroups", "No Groups"),
    method = method %>% 
  ...
  dplyr::filter(status %in% c("Increased", "Decreasd")) %>% 
  droplevels() %>% 
  ...
  scale_x_continuous(
    expand = expansion(c(0, 0.05)), breaks = seq(0, 1, by = 0.2),
    labels = percent
  ) +
