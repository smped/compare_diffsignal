...
db_ls_results <-  dba_ls %>% 
  ...
  sortSeqlevels() %>% 
  sort() 
db_ls_results$status <- addDiffStatus(db_ls_results, fc_col = "Fold")$status
...
db_tmm_results <- dba_tmm %>%
  ...
  sortSeqlevels() %>%
  sort()
db_tmm_results$status <- addDiffStatus(db_tmm_results, fc_col = "Fold")$status
...
p <- GRangesList(
  DiffBind = select(db_ls_results, logFC = Fold, status), 
...
p <- GRangesList(
  DiffBind = select(db_ls_results, PValue = `p-value`, status), 
...
p <- GRangesList(
  DiffBind = select(db_tmm_results, logFC = Fold, status), 
...
p <- GRangesList(
  DiffBind = select(db_tmm_results, PValue = `p-value`, status), 
...
rp <- readParam(
  ...
  discard = c(greylist, hg19.blacklist)
)
bfl <- path(bfl) %>% 
  dirname() %>% 
  unique() %>% 
...
sw_ls_results <- sw_ls_all %>% 
  mergeByHMP(inc_cols = "overlaps_ref") %>% 
  filter(n_windows > 1) %>% 
...
![*MA-plot for sliding-window results using Library Size Normalisation*](`r href`)
...
![*MA-plot for sliding-window results using TMM Normalisation*](`r href`)
...
![*MA-plot for sliding-window results using TMM Normalisation within treatment groups*](`r href`)
...
p <- sw_sq_all %>% 
  ...
  geom_smooth(se = FALSE) +
  geom_hline(yintercept = 0) +
  ggtitle(glue("{target}: Sliding Windows"), subtitle = "SQ Normalisation") +
  labs(x = "Ave Signal", y = "logFC") 
if (interactive()) {
  p
} else {
...
p <- sw_sqgc_all %>% 
  ...
  geom_smooth(se = FALSE) +
  geom_hline(yintercept = 0) +
  ggtitle(glue("{target}: Sliding Windows"), subtitle = "SQ-GC Normalisation") +
  labs(x = "Ave Signal", y = "logFC") 
if (interactive()) {
  p
} else {
