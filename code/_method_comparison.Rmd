...
# params <- list(
#   dataset = "PRJNA509779", target = "H3K27ac", cell_line = "ZR-75-1",
...
```{r bfl}
bfl <- samples %>% 
  mutate(path = here::here("data", "bam", dataset, glue("{accession}.bam"))) %>% 
  pull(path) %>% 
  BamFileList()
sq <- seqinfo(bfl) %>% 
...
```{r tbl-samples, results='asis'}
samples %>% 
  pander(
    caption = "Description of samples used for method testing"
  )
```
...
```{r plot-db-read-densities}
db_se <- dba(dba, bSummarizedExperiment = TRUE)
p <- plotAssayDensities(
  db_se, assay = "Reads", trans = "log1p", colour = "Treatment"
  ) +
...
![*Read Density Plot for counts returned by DiffBind*](`r href`)
...
colData(se) <- colData(se) %>%
  ...
  mutate(
    accession = str_remove_all(sample, ".bam"),
  ) %>%
  ...
  DataFrame()
# se <- dba(dba_ls, bSummarizedExperiment = TRUE)
# assayNames(se) <- str_replace_all(assayNames(se), "^Reads$", "counts")
# se$treatment <- factor(se$Treatment, levels = treat_levels)
# se$totals <- as.numeric(se$Reads)
...
```{r plot-ec-read-densities}
p <- plotAssayDensities(se, trans = "log1p", colour = "treatment") +
  scale_colour_manual(values = c("deepskyblue3", "red3"))
if (interactive()) {
  p
} else {
...
![*Read Density Plot for counts returned by csaw*](`r href`)
...
p <- ec_ls_results %>% 
  ...
  geom_smooth(se = FALSE) +
  geom_hline(yintercept = 0) +
  ggtitle(glue("{dataset}: {target}"), subtitle = "extraChIPs; Fixed Width; Library Size Normalisation") +
  labs(colour = "Status") +
  scale_colour_manual(values = status_colours)
if (interactive()) {
  p
...
![*GC-bias plot for extraChIPs results using Library Size Normalisation*](`r href`)
...
p <- ec_tmm_results %>% 
  ...
  geom_smooth(se = FALSE) +
  geom_hline(yintercept = 0) +
  ggtitle(glue("{dataset}: {target}"), subtitle = "extraChIPs; Fixed Width; TMM Normalisation") +
  labs(colour = "Status") +
  scale_colour_manual(values = status_colours)
if (interactive()) {
  p
...
p <- ec_tmmg_results %>% 
  ...
  geom_smooth(se = FALSE) +
  geom_hline(yintercept = 0) +
  ggtitle(glue("{dataset}: {target}"), subtitle = "extraChIPs; Fixed Width; TMM-Groups Normalisation") +
  labs(colour = "Status") +
  scale_colour_manual(values = status_colours)
if (interactive()) {
  p
...
![*GC-bias plot for extraChIPs results using TMM-Groups Normalisation*](`r href`)
...
p <- cqn_results %>% 
  ...
  geom_smooth(se = FALSE) +
  geom_hline(yintercept = 0) +
  ggtitle(glue("{dataset}: {target}"), subtitle = "extraChIPs; Fixed Width; CQ Normalisation") +
  labs(colour = "Status") +
  scale_colour_manual(values = status_colours)
if (interactive()) {
  p
...
![*MA-plot for extraChIPs results using CQ Normalisation*](`r href`)
...
![*GC-bias plot for extraChIPs results using CQ Normalisation*](`r href`)
...
cqn_groups <- treat_levels %>% 
  ...
  setNames(treat_levels)
offset <- do.call("cbind", lapply(cqn_groups, function(x) x$glm.offset))[,colnames(se)]
...
p <- cqng_results %>% 
  ...
  geom_smooth(se = FALSE) +
  geom_hline(yintercept = 0) +
  ggtitle(glue("{dataset}: {target}"), subtitle = "extraChIPs; Fixed Width; CQN-Groups") +
  labs(colour = "Status") +
  scale_colour_manual(values = status_colours)
if (interactive()) {
  p
...
![*MA-plot for extraChIPs results using CQ Normalisation within groups*](`r href`)
...
![*GC-bias plot for extraChIPs results using CQN-groups*](`r href`)
...
p <- sqn_results %>% 
  ...
  geom_smooth(se = FALSE) +
  geom_hline(yintercept = 0) +
  ggtitle(glue("{dataset}: {target}"), subtitle = "extraChIPs; Fixed Width; Smooth Quantile Normalisation") +
  labs(colour = "Status") +
  scale_colour_manual(values = status_colours)
if (interactive()) {
  p
...
p <- sqn_results %>% 
  ...
  geom_smooth(se = FALSE) +
  geom_hline(yintercept = 0) +
  ggtitle(glue("{dataset}: {target}"), subtitle = "extraChIPs; Fixed Width; SQ Normalisation") +
  labs(x = "% GC", colour = "Status") +
  scale_x_continuous(labels = percent) +
  scale_colour_manual(values = status_colours)
if (interactive()) {
...
![*GC-bias plot for extraChIPs results using SQN*](`r href`)
...
p <- sqngc_results %>% 
  ...
  geom_smooth(se = FALSE) +
  geom_hline(yintercept = 0) +
  ggtitle(glue("{dataset}: {target}"), subtitle = "extraChIPs; Fixed Width; SQ-GC Normalisation") +
  labs(colour = "Status") +
  scale_colour_manual(values = status_colours)
if (interactive()) {
  p
...
![*GC-bias plot for extraChIPs results using SQ-GC Normalisation*](`r href`)
...
```{r ec-lt-fixed}
assay(se, "logCPM") <- cpm(assay(se, "counts"), log = TRUE, lib.size = se$totals)
lt_results <- fitAssayDiff(
  se, assay = "logCPM", method = "lt", design = X, asRanges = TRUE
) %>% 
  addDiffStatus()
```


This analysis returned `r comma(sum(lt_results$FDR < 0.05))` peaks considered to show evidence of differential signal.


##### MA Plot

```{r plotma-ec-lt-fixed}
p <- lt_results %>% 
  as_tibble() %>% 
  ggplot(aes(logCPM, logFC)) +
  geom_point(aes(colour = status), alpha = 0.7) +
  geom_smooth(se = FALSE) +
  geom_hline(yintercept = 0) +
  ggtitle(glue("{dataset}: {target}"), subtitle = "extraChIPs; Fixed Width; Limma-Trend") +
  labs(colour = "Status") +
  scale_colour_manual(values = status_colours)
if (interactive()) {
  p
...
![*MA-plot for extraChIPs results using Limma-Trend*](`r href`)

##### GC Bias Plot

```{r plotgc-ec-lt-fixed}
p <- lt_results %>% 
  as_tibble() %>% 
  ggplot(aes(gc, logFC)) +
  geom_point(aes(colour = status), alpha = 0.7) +
...
![*GC-bias plot for extraChIPs results using Limma-Trend*](`r href`)
...
It should be noted that DiffBind re-centres any supplied peaks whilst extraChIPs will not.
This accounts for much of the observed difference, and direct comparison of Library Size normalisation using the two identical sets of ranges (i.e. passing DiffBind re-cented ranges to extraChIPs) does in fact yield a 1:1 association between logFC estimates and detection of differential signal.
The following comparison instead indirectly assesses the impact of this additional step performed by DiffBind.
...
```{r set-windows}
window_size <- round(params$width / 5, -1)
window_step <- ceiling(0.1 * window_size / 3) * 10
bfl <- path(bfl) %>% 
  dirname() %>% 
  unique() %>% 
  file.path(glue("{samples$input}.bam")) %>% 
  unique() %>% 
  BamFileList() %>% 
...
Windows were set to be about 1/5 of the fixed-width window-size, with each range covered up to 3 times.
...
```{r plot-sw-read-densities}
p <- plotAssayDensities(filtcounts, trans = "log1p", colour = "treatment") +
  scale_colour_manual(values = c("deepskyblue3", "red3"))
if (interactive()) {
  p
} else {
...
![*Read Density Plot for sliding-window counts returned by extraChIPs after filtering*](`r href`)
...
p <- sw_ls_all %>% 
  ...
  ggtitle(glue("{dataset}: {target}"), subtitle = "Sliding Windows; Library Size Normalisation") +
...
p <- sw_tmm_all %>% 
  ...
  geom_hline(yintercept = 0) +
  scale_colour_manual(values = c("grey", "red")) +
  ggtitle(glue("{dataset}: {target}"), subtitle = "Sliding Windows; TMM Normalisation") +
  labs(x = "logCPM", y = "logFC") 
if (interactive()) {
  p
} else {
...
p <- sw_tmmg_all %>% 
  ...
  geom_hline(yintercept = 0) +
  scale_colour_manual(values = c("grey", "red")) +
  ggtitle(glue("{dataset}: {target}"), subtitle = "Sliding Windows; TMM-Groups Normalisation") +
  labs(x = "logCPM", y = "logFC") 
if (interactive()) {
  p
} else {
...
p <- sw_sq_all %>% 
  ...
  geom_hline(yintercept = 0) +
  scale_colour_manual(values = c("grey", "red")) +
  ggtitle(glue("{dataset}: {target}"), subtitle = "Sliding Windows; SQN (Counts)") +
  labs(x = "logCPM", y = "logFC") 
if (interactive()) {
  p
} else {
...
p <- sw_sqgc_all %>% 
  ...
  geom_hline(yintercept = 0) +
  scale_colour_manual(values = c("grey", "red")) +
  ggtitle(glue("{dataset}: {target}"), subtitle = "Sliding Windows; SQN-GC (Counts)") +
  labs(x = "logCPM", y = "logFC") 
if (interactive()) {
  p
} else {
...
This analysis returned `r comma(sum(sw_cqn_results$FDR < 0.05))` peaks considered to show evidence of differential signal.
...
p <- sw_cqn_all %>% 
  ...
  geom_hline(yintercept = 0) +
  scale_colour_manual(values = c("grey", "red")) +
  ggtitle(glue("{dataset}: {target}"), subtitle = "Sliding Windows; CQN") +
  labs(x = "logCPM", y = "logFC") 
if (interactive()) {
  p
} else {
...
p <- sw_cqng_all %>% 
  ...
  geom_hline(yintercept = 0) +
  scale_colour_manual(values = c("grey", "red")) +
  ggtitle(glue("{dataset}: {target}"), subtitle = "Sliding Windows; CQN-Groups") +
  labs(x = "logCPM", y = "logFC") 
if (interactive()) {
  p
} else {
...
p <- sw_lt_all %>% 
  ...
  geom_hline(yintercept = 0) +
  scale_colour_manual(values = c("grey", "red")) +
  ggtitle(glue("{dataset}: {target}"), subtitle = "Sliding Windows; Limma-Trend") +
  labs(x = "logCPM", y = "logFC") 
if (interactive()) {
  p
} else {
...
p <- sw_ltsq_all %>% 
  ...
  geom_hline(yintercept = 0) +
  scale_colour_manual(values = c("grey", "red")) +
  ggtitle(glue("{dataset}: {target}"), subtitle = "Sliding Windows; SQN + Limma-Trend") +
  labs(x = "logCPM", y = "logFC") 
if (interactive()) {
  p
} else {
...
p <- sw_ltsqgc_all %>% 
  ...
  geom_hline(yintercept = 0) +
  scale_colour_manual(values = c("grey", "red")) +
  ggtitle(glue("{dataset}: {target}"), subtitle = "Sliding Windows; SQN-GC + Limma-Trend") +
  labs(x = "logCPM", y = "logFC") 
if (interactive()) {
  p
} else {
...
## Fixed Vs Sliding Windows {.tabset}

As representative of a conservative approach without known bias, the fixed and sliding window approaches were compared using *library size normalisation*.
This gives a guide as to which sites are additionally covered or omitted by either approach.

### MA Plots

```{r plotma-fixed-sliding}
A <- ec_ls_results %>% 
  mutate(inSliding = overlapsAny(., sw_ls_results)) %>% 
  as_tibble() %>% 
  mutate(inSliding = ifelse(inSliding, "In Sliding Windows", "Not In Sliding Windows")) %>% 
  ggplot(aes(logCPM, logFC)) +
  geom_point(aes(colour = status)) +
  facet_wrap(~inSliding) +
  scale_colour_manual(values = status_colours) +
  labs(colour = "Status")
B <- sw_ls_results %>% 
    mutate(inFixed = overlapsAny(., ec_ls_results)) %>% 
  as_tibble() %>% 
  mutate(inFixed = ifelse(inFixed, "In Fixed Windows", "Not In Fixed Windows")) %>% 
  ggplot(aes(logCPM, logFC)) +
  geom_point(aes(colour = status)) +
  facet_wrap(~inFixed) +
  scale_colour_manual(values = status_colours) +
...
p <- A + B + 
  ...
  plot_annotation(title = glue("{dataset}: {target}"))
if (interactive()) {
  p
} else {
  lb <- knitr::opts_current$get("label")
...
![*MA Plots for each analysis with points separated by whether they were detected or not in the contrasting approach.*](`r href`)

### logFC Comparison

```{r logfc-fixed-sliding}
p <- GRangesList(
  Fixed = select(ec_ls_results, logCPM, logFC, status),
  Sliding = select(sw_ls_results, logCPM, logFC, status)
) %>% 
  plotPairwise(var = "logFC", colour = "status") +
  ggtitle(glue("{dataset}: {target}")) +
  scale_fill_manual(
    values = c(
      setNames(status_colours, paste("Fixed", names(status_colours))),
      setNames(status_colours, paste("Sliding", names(status_colours)))
    )
  )
if (interactive()) {
  p
...
![*Comparison of logFC estimates across fixed and sliding windows including status from each dataset.*](`r href`)

### logCPM Comparison

```{r logcpm-fixed-sliding}
p <- GRangesList(
  Fixed = select(ec_ls_results, logCPM, logFC, status),
  Sliding = select(sw_ls_results, logCPM, logFC, status)
) %>% 
  plotPairwise(var = "logCPM", alpha = 0.7) +
  ggtitle(glue("{dataset}: {target}")) 
if (interactive()) {
  p
} else {
...
![*Comparison of logCPM values across fixed and sliding windows indicating detection/inclusion status under either approach*](`r href`)

### Status Comparison


```{r status-fixed-sliding}
p <- GRangesList(
  Fixed = select(ec_ls_results, logCPM, logFC, status),
  Sliding = select(sw_ls_results, logCPM, logFC, status)
) %>% 
  ...
  summarise(n = dplyr::n(), .by = ends_with("status")) %>% 
  mutate(combined = fct_cross(fct_rev(Fixed_status), fct_rev(Sliding_status), sep = " - ")) %>% 
  ggplot(aes(Fixed_status, Sliding_status, fill = combined)) +
  geom_point(aes(size = n), shape = 21) +
  geom_label(aes(label = comma(n)), fill = "white", alpha = 0.7) +
  geom_rect(xmin = 0.6, xmax = 1.4, ymin = 1.6, ymax = 3.4, colour = "red", fill = NA) +
  geom_rect(ymin = 0.6, ymax = 1.4, xmin = 1.6, xmax = 3.4, colour = "red", fill = NA) +
  geom_rect(ymin = 3.6, ymax = 4.4, xmin = 1.6, xmax = 3.4, colour = "blue", fill = NA) +
  geom_rect(xmin = 3.6, xmax = 4.4, ymin = 1.6, ymax = 3.4, colour = "blue", fill = NA) +
  scale_x_discrete(drop = FALSE) +
  scale_y_discrete(drop = FALSE) +
  scale_size(range = c(5, 55)) +
  scale_fill_viridis_d(direction = -1) +
  guides(size = "none") +
  ggtitle(glue("{dataset}: {target}")) +
  labs(x = "Fixed Window Status", y = "Slding Window Status", fill = "Combined Status")
if (interactive()) {
  p
} else {
  lb <- knitr::opts_current$get("label")
...
![*Comparison of status between fixed and sliding windows. Points along the diagonal are consistent between approaches. Red rectangles highlight the regions considered unchanged under one approach, but changed in the other. Blue rectangles highlight the regions considered changed under one approach, but which were undetectable/excluded from the alternative method*](`r href`)
...
qtest <- list(
  `FW-Counts` = quantro(assay(se, "counts"), groupFactor = se$treatment, B = 1e3),
  `FW-logCPM` = quantro(assay(se, "logCPM"), groupFactor = se$treatment, B = 1e3),
  `SW-Counts` = quantro(assay(filtcounts, "counts"), groupFactor = filtcounts$treatment, B = 1e3),
  `SW-logCPM` = quantro(assay(filtcounts, "logCPM"), groupFactor = filtcounts$treatment, B = 1e3)
)
qtest %>% 
  lapply(
    function(x){
      p <- anova(x)$`Pr(>F)`[[1]]
      perm <- x@quantroPvalPerm
      tibble(`Anova P` = p, `Permutation P` = perm)
    }
  ) %>% 
  bind_rows(.id = "method") %>% 
  mutate(
    windows = ifelse(grepl("^FW", method), "Fixed", "Sliding"),
    assay = str_remove_all(method, "^.+-"),
    `Group Effects` = `Anova P` < 0.05 | `Permutation P` < 0.05
  ) %>% 
  dplyr::select(
    method, windows, assay, ends_with("P"), everything()
  ) %>% 
  rename_with(str_to_title) %>% 
  pander(
    justify = "lllrrl",
    caption = "
    Summary of quantro test results. The Anova results test for equality of 
    medians between groups, whilst the permutation test identifies distributions 
    with different shapes. Any difference detected between treatment groups 
    will raise concerns about methods, such as TMM and CQN, which do not 
    explicitly account for differences in data distributions.
    "
  )
```
...
```{r plot-all-cat, fig.height=10}
abs_percent <- function(x, ...) scales::percent(abs(x), ...)
p <- all_res %>%
  lapply(
    function(x) {
      tbl <- table(x$status)
  ...
  mutate(p = ifelse(status == "Decreased", -1, 1) * p) %>% 
  arrange(desc(p)) %>% 
  mutate(
    windows = ifelse(grepl("^SW", method), "Sliding Windows", "Fixed Width"),
    groups = ifelse(grepl("(SQN|G$)", method), "Explicit Groups", "No Groups"),
    method = method %>% 
  ...
  geom_col() +
  facet_grid(windows + groups ~ ., scales = "free", space = "free", switch = "y") +
  ggtitle(glue("{dataset}: {target} Method Comparison")) +
  labs(x = "% All Regions", y = "Method", fill = "Status") +
  scale_x_continuous(
