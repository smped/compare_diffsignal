...
tbl <- tibble(
  ...
  `PRJNA509779 (AR)` = case_when(
    ...
    method %in% c(
      paste("DB", "LS", sep = "-"), 
      paste("FW", c("LS", "TMMG", "CQNG"), sep = "-"),
      paste("SW", c("CQNG", "LTSQN", "LT", "LS", "TMMG"), sep = "-")
    ) ~ 1,
    TRUE ~ 0
  ),
  ...
  `PRJNA509779 (H3K27ac)` = case_when(
    ...
    method %in% c(
      paste("DB", c("LS", "TMM"), sep = "-"), 
      paste("FW", c("LS", "TMM", "TMMG", "CQN", "SQN", "SQNGC", "LT"), sep = "-"),
      paste("SW", c(), sep = "-")
    ) ~ 1,
    TRUE ~ 0
  )
  ...
  reactable(
    sortable = TRUE, filterable = FALSE, pagination = FALSE,
    columns = list(
      method = colDef(
      ...
      score = colDef(name = "Score", footer = "", maxWidth = 80),
